...
def generate_launch_description():
    ...
    default_map_filename = os.path.join(pkg_share, 'maps', 'car_bot_map')
    robot_controllers = os.path.join(pkg_share, 'config', 'ros2_controllers.yaml')
...
# ...
    ...
    load_joint_state_broadcaster = TimerAction(
        period=5.0, # 必须在 spawn_entity 之后运行，等待 Gazebo 加载完毕
        actions=[Node(
            package="controller_manager",
            executable="spawner",
    ...
    # 2. 启动机械臂控制器 (名称必须与 ros2_controllers.yaml 中定义的一致)
    load_arm_controller = TimerAction(
        period=7.0, # 稍微延后一点，确保 broadcaster 先启动
        actions=[Node(
            package="controller_manager",
            executable="spawner",
    ...
    ld = LaunchDescription([
        ...
        # 这里的顺序很重要：
        load_joint_state_broadcaster, # 先启动状态广播
        load_arm_controller,          # 再启动机械臂控制
        load_gripper_controller,      # 启动夹爪控制
